...
async function executeMutation(
  ...
  if (type === MutationTypes.CREATE || type === MutationTypes.DELETE || type === MutationTypes.DEVALIDATINGUPDATE) {
  ...
  const { mutate } = useMutation(mutation, () => ({
    ...
    update: (cache: ApolloCache<any>, { data: changeData }) => {
      // For devalidating updates, fully refetch all affected queries
      if(type === MutationTypes.DEVALIDATINGUPDATE){
        void refetchAffectedQueries(
          affectedQueries,
          mutationObject
        );
      } else {
 ...
 * @returns {void}
 */
function updateAffectedQueries(
  ...
  affectedQueries.forEach((queryObject) => {
    if (!mutationObject.cacheLocation) {
      throw new Error(
        i18n.global.t('errors.cache_location_missing') +
          JSON.stringify(mutationObject)
      );
    }

    const change: Record<string, unknown> =
      changes[mutationObject.cacheLocation] ?? {};

    // Read existing query from cache
    const data: Record<string, Array<Record<string, unknown>>> | null =
      cache.readQuery({ query: queryObject.query });
    ...
    if (data) {
      ...
      // Case 2: DELETE (removes object from cache)
        ...
        newData = oldData.filter(
          (dataPoint: Record<string, unknown>) => dataPoint.uuid !== change.uuid
        );
      }
