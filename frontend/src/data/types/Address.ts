...
export class Address{
  ...
  validate(): boolean{
    try {
      Joi.assert(this, AddressItem as Schema)
      return true;
    } catch (e) {
      return false
    }
   ...
   * @returns {void}
   */
  replace({street, number, city, zipCode}: {street: string, number: string, city: string, zipCode: string}): void{
    this.street = street
    this.number = number
    this.city = city
   ...
   */
  prettyString(): string {
    if ([this.street, this.number, this.city, this.zipCode].every((val) => val !== null)) {
