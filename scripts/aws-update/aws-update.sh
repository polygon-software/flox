...
then
  ...
  then
    ...
    terraform apply -target="module.web-spa-pwa[0].aws_cloudfront_distribution.website_distribution" -target="module.api-serverless[0].aws_lambda_function.api_lambda" -auto-approve -var-file="../../support/flox.tfvars"
  else
    terraform apply -target="module.web-spa-pwa[0].aws_cloudfront_distribution.website_distribution" -target="module.api-ebs[0].aws_elastic_beanstalk_environment.api_env" -auto-approve -var-file="../../support/flox.tfvars"
  fi
# For SSR mode, also redeploy SSR frontend
  ...
  if [[ $serverless_api == true ]]
    then
      # In serverless API mode, also renew lambda
      terraform apply -target="module.web_ssr[0].aws_elastic_beanstalk_environment.frontend_env" -target="module.api-serverless[0].aws_lambda_function.api_lambda" -auto-approve -var-file="../../support/flox.tfvars"
    else
      terraform apply -target="module.api-ebs[0].aws_elastic_beanstalk_environment.api_env" -target="module.web_ssr[0].aws_elastic_beanstalk_environment.frontend_env" -auto-approve -var-file="../../support/flox.tfvars"
    fi
fi
