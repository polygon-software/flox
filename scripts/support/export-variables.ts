...
console.log(
  `aws_s3_region="${
    infrastructureConfig.aws_s3_region ?? infrastructureConfig.aws_region
  }"`
);
...
console.log(
  `serverless_db_version="${infrastructureConfig.serverless_db_version}"`
);

console.log("# ======== Frontend Config options ========");
const frontend_mode_key = `mode_${mode}`;
console.log(`frontend_build_mode="${frontendConfig[frontend_mode_key]}"`);

console.log("# ======== Auth module options ========");
// 2FA settings
console.log(
  `mfa_configuration=${authOptions?.useTwoFactor ? '"ON"' : '"OFF"'}`
);

// E-mail as username settings
console.log(
  `auto_verified_attributes=${
    authOptions?.emailAsUsername ? '["email"]' : "[]"
  }`
);
console.log(
  `username_attributes=${authOptions?.emailAsUsername ? '["email"]' : "[]"}`
);
...
console.log(
  `database_master_username="${backendConfig.database_master_username}"`
);
